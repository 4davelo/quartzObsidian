---
type: document-section
document_id: la-technique-du-batiment-tous-corps-detat
section_id: "section:15.3"
ordre_document: 116
titre: "15.3 Définitions incontournables"
aliases: ["La Technique du bâtiment – Tous corps d’état — 116 15.3 Définitions incontournables"]
resume_section: |-
  **15.3 Définitions incontournables**

  - **Éléments structuraux** : Définitions des composants d'un escalier, incluant trémie, volée, palier, paillasse, rampant, limon, crémaillère, mur d’échiffre, noyau et jour d’escalier.

  - **Protection contre les chutes** : Norme NF P 01-012 définissant les hauteurs de protection des rampes (90 cm sur volée, 100 cm sur palier, ajustable à 90 cm si le jour ≤ 60 cm). Références aux rampes pleines et ajourées.

  - **Éléments de calcul** : Dimensions clés des escaliers (hauteur de marche, giron, nez de marche, emmarchement, ligne de foulée, collet, échappée, montée). Normes spécifiques pour maisons individuelles et habitations collectives.
  - **15.3.1 Éléments structuraux** — `section:15.3.1`, heading, profondeur 5, pages 776-777
  - **15.3.2 Protection contre les chutes** — `section:15.3.2`, heading, profondeur 5, pages 778-779
  - **15.3.3 Éléments de calcul** — `section:15.3.3`, heading, profondeur 5, pages 780-781
document_parent: "[[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/index|La Technique du bâtiment – Tous corps d’état — Index]]"
section_precedente: "[[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/115-15-2-passage-du-brancard-et-accessibilite-des-immeubles-collectifs-aux-personnes-a-mobilite-red|15.2 Passage du brancard et accessibilité des immeubles collectifs aux personnes à mobilité réduite]]"
section_suivante: "[[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/117-15-4-regles-de-conception|15.4 Règles de conception]]"
tags: [document-section, ouvrage, btp, tous-corps-detat]
sources: [116_15.3_définitions_incontournables.md]
created: 2026-04-29
updated: 2026-04-29
contextes: [appel-doffres, execution-projet, technique]
autorite: [pratique, reference, reglementaire, normatif]
juridictions: [france, generique, ue]
familles_sources: [ouvrage, guide, norme, reglementation]
---

- **Protection contre les chutes** : Norme NF P 01-012 définissant les hauteurs de protection des rampes (90 cm sur volée, 100 cm sur palier, ajustable à 90 cm si le jour ≤ 60 cm). Références aux rampes pleines et ajourées.

- **Éléments de calcul** : Dimensions clés des escaliers (hauteur de marche, giron, nez de marche, emmarchement, ligne de foulée, collet, échappée, montée). Normes spécifiques pour maisons individuelles et habitations collectives.

15.3 Définitions incontournables** 

**15.3.1 Éléments structuraux** 

Ils font l’objet de la **fgure [15.4](117_15.4_règles_de_conception.md)** : trémie d’escalier ou cage d’escalier : vide de construction dans lequel sont implantés les volées d’escalier et les paliers ; volée d’escalier : ensemble de marches (25 au maximum) compris entre deux paliers consécutifs ; palier : plateforme constituant un repos entre deux volées intermédiaires ; paillasse : dalle inclinée en béton armé incorporant marches et contremarches dans la masse ; rampant : sous-face de la paillasse ;

![[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/images/La_Technique_du_bâtiment_-_Tous_corps_d’état_--_Cécile_Granier_&_Michel_Platzer_&_Daniel_Montharry_--_2024_--_Le_Moniteur.epub-0777-00.png]]

limon : poutre inclinée qui sert d’appui aux marches et permet de franchir la hauteur entre deux paliers ; crémaillère : poutre inclinée, centrale ou latérale, dont la face supérieure est à redans et qui porte les marches de toute une volée ; mur d’échiffre : mur supportant latéralement les marches ; noyau d’escalier : partie centrale pleine de la cage d’escalier sur laquelle viennent s’appuyer les marches ; le noyau peut être constitué par un voile dans le cas des escaliers à volée droite ou par un poteau dans le cas de l’escalier hélicoïdal ; jour d’escalier : espace central autour duquel se développe un escalier. 

**15.3.2 Protection contre les chutes** 

La norme NF P 01-012 fixe la hauteur de protection pour les rampes pleines à 90 cm sur volée d’escalier et à 100 cm sur palier dans la zone où la main courante est horizontale, mais si la largeur du jour d’escalier est égale ou inférieure à 60 cm, la hauteur de protection peut être ramenée à 90 cm. Pour les rampes ajourées, on se reportera à la **fgure [15.5](118_15.5_escaliers_en_béton.md)** .

![[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/images/La_Technique_du_bâtiment_-_Tous_corps_d’état_--_Cécile_Granier_&_Michel_Platzer_&_Daniel_Montharry_--_2024_--_Le_Moniteur.epub-0780-00.png]]


_**Fig. 15.5. Rampes ajourées**_ 

NF P 01-012 (juillet 1988 – indice de classement : P 01-012) : Dimensions des garde-corps – Règles de sécurité relatives aux dimensions des garde-corps et rampes d’escalier. 

**15.3.3 Éléments de calcul** 

Ils font l’objet de la **fgure [15.6](119_15.6_escaliers_en_bois.md)** : hauteur de marche _H_ : différence de niveau entre deux marches successives, comprise entre 13 et 17 cm ; giron _G_ : profondeur de marche, mesurée sur la ligne de foulée, comprise entre 25 et 32 cm ; nez de marche : partie de la marche en saillie de 2 à 3 cm par rapport à la contremarche ; emmarchement _E_ : c’est la largeur de la marche, égale en général à 90 cm pour les maisons individuelles et à 120 cm pour les habitations collectives ;

![[documents/Livres/BTP/la-technique-du-batiment-tous-corps-detat/images/La_Technique_du_bâtiment_-_Tous_corps_d’état_--_Cécile_Granier_&_Michel_Platzer_&_Daniel_Montharry_--_2024_--_Le_Moniteur.epub-0782-00.png]]


_**Fig. 15.6. Éléments de calcul**_ 

ligne de foulée : représente en plan le parcours d’une personne qui utilise l’escalier en se tenant à la rampe du côté du jour ; elle est située à mi-emmarchement si celui-ci est inférieur à 100 cm, ou à 50 cm du collet si l’emmarchement est supérieur à 100 cm ; 

collet : représente en plan la partie la plus étroite d’une marche d’un escalier balancé et ne sera jamais inférieur à 10 cm si l’on compte un double quartier tournant, ou à 15 cm si l’on dispose d’un simple quartier tournant ; 

échappée : hauteur libre entre le nez des marches et la sousface de la volée supérieure ou d’une poutre bordant la trémie d’escalier. Elle est en général égale à 200 cm ; 

montée : hauteur d’étage de sol à sol à monter, généralement fixée à 270 cm. 

**